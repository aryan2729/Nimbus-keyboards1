  ...
  useGSAP(()=>{
    ...
    return ()=> {
      if(buttonRef.current){
          buttonRef.current?.removeEventListener("mouseover", handleMouseMove);
    ...
    >
      ...
      <FadeIn className="relative mx-auto max-w-7xl px-4 text-center" targetChildren>
        ...
        <button 
          ...
          disabled={isPressed}
          className={clsx(
          "group motion-safe:transition-all relative w-full overflow-hidden rounded-full border-8 border-gray-900 bg-linear-to-r/oklch from-sky-300 to-sky-600 px-6 py-6 ease-out focus:ring-[24px] focus:ring-sky-500/50 focus:outline-none motion-safe:duration-300 md:border-[12px] md:px-20 md:py-16",
          ...
          <div className="absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/40 to-transparent ease-out group-hover:translate-x-full motion-safe:transition-transform motion-safe:duration-1000" />
            <div className="relative z-10 flex items-center justify-center gap-6 md:gap-8 ">
              ...
              {isPressed && (<div className="hidden group-hover:translate-x-2 group-hover:scale-125 motion-safe:transition-all motion-safe:duration-300 md:block">
