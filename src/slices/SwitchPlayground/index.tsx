      ...
      <FadeIn>
        ...
        <FadeIn targetChildren className="grid grid-cols-1 gap-4 overflow-hidden sm:grid-cols-2 ">
          ...
          {slice.primary.switches.map((item) => isFilled.contentRelationship(item.switch) ?  (
            <SharedCanvas key={item.switch.id} color={item.switch}  />)  : null
            )}
        </FadeIn>
      </FadeIn>
    </Bounded>
...
const SharedCanvas = ({color} : SharedCanvasProps) =>{
  ...
  return (
    <div className="group relative min-h-96 overflow-hidden rounded-3xl select-none">
      ...
      <button 
        onClick={handleSound}
        className="font-bold-slanted absolute bottom-0 left-0 z-10 flex items-center gap-3 p-6 text-4xl text-white uppercase focus:ring-2 focus:ring-white focus:outline-non">
        {name} <LuVolume2/>
      </button>
